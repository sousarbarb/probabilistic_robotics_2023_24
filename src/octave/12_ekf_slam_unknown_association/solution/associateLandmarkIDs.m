...
function observations = associateLandmarkIDs(mu, sigma, observations)
  ...
  id_state = 1;
    ...
    %compute the covariance and its inverse
		sigma_zx = eye(2,2)*0.01; %TODO minimum uncertainty (check dimensions)
		sigma_nn = C*sigma*C'+sigma_zx;  %TODO covariance for the current landmark

    %compute information matrix
		omega_nn = inv(sigma_nn);%TODO canonical representation of sigma_nn
    ...
    %for all measurements
      ...
      measurement = measurements(m);
			...
			A(m, n) = (z-h)'*omega_nn*(z-h);%TODO likelihood with information matrix (omega)
		endfor
  ...
  for m=1:M
    ...
    %if the association passes the gate
		if(a_mn < gating_tau)%TODO add gating condition for this association
		
			%add the possible association - creating the associations vector
      %[measurement id, proposed landmark id , association matrix value a_mn] 
			associations = [associations; [m, min_index, a_mn]]; %TODO add the information to our bookkeeping
		endif
  endfor
 ...
 number_of_gated_associations = size(associations, 1);
    ...
    %if the association is not the minimum in the column
		if(a_mn != min_on_column)%TODO check if it is NOT a best friend
	...
	if(M > 1)
      ...
      %this association is doubtful, skip evaluation
			...
			if((second_row_min_value - a_mn) < lonely_best_friend_gamma_threshold ||
				(second_col_min_value - a_mn) < lonely_best_friend_gamma_threshold)%TODO check if it is NOT a lonely best friend
