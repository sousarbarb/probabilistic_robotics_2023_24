...
function [mu, sigma] = correction(sigmaP, weightsM, weightsC, landmarks, observations)
	...
	for i=1:num_landmarks_seen
		...
		z_t = [z_t; measurement.x_pose; measurement.y_pose]; % where we see landmark, stack on vec
	...
	%sigma = sigma - K*(Sigma_z + Sigma_noise)*K'; THIS IS ALSO EQUIVALENT!
end
